...
from Bio.PDB import parse_pdb_header, Structure, Model, Chain, Residue, Atom
...
def ConvertToChainList(self, pdb_obj, **kwargs):
	...
	if isinstance(pdb_obj, str) or 'filename' in kwargs:
		...
		# Needs to add **kwargs
		parser = PDBParser(QUIET = kwargs.get('QUIET', True), PERMISSIVE = kwargs.get('PERMISSIVE', True))
